...
package id.hardana.entity.sys.log;
 ...
 @NamedQueries({
     ...
     @NamedQuery(name = "LoginAdmin.findById", query = "SELECT l FROM LoginAdmin l WHERE l.id = :id"),
     @NamedQuery(name = "LoginAdmin.findActiveUserByChannel", query = "SELECT l FROM LoginAdmin l WHERE l.channelId = :channelId AND l.userId = :userId AND l.status = :status")})
 public class LoginAdmin implements Serializable {
     ...
     public String getSessionExpired() {
         return sessionExpired == null ? null : DATE_FORMAT.format(sessionExpired);
     }
     ...
     public String toString() {
         return "id.hardana.entity.api.LoginAdmin[ id=" + id + " ]";
     }
